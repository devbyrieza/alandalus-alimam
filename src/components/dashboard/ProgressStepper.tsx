                        ...
                        <div key={idx} className="flex flex-col items-center relative flex-1">
                            ...
                            {idx < STEPS.length - 1 && (
                                <div className="absolute top-5 left-[50%] right-[-50%] h-0.5 bg-stone-100 z-0">
                                    <div
                                        className={`h-full bg-teal-500 transition-all duration-700 ${idx < activeIndex ? 'w-full' : 'w-0'}`}
                                    />
                                </div>
                            )}
                            ...
                            {/* Label */}
                            <span
                                className={`mt-3 text-[10px] sm:text-xs font-black uppercase tracking-wider transition-colors duration-500 ${isActive ? "text-teal-700" : isCompleted ? "text-stone-600" : "text-stone-400"
                                    }`}
                            >
                                {step.label}
                            </span>
                            ...
                            {isActive && (
                                <div className="mt-1 h-1 w-8 bg-teal-500 rounded-full animate-pulse" />
                            )}
                        </div>
                    );
                })}
