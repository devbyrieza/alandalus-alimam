...
export default function ProcessSection() {
  return (
    <section id="proses" className="py-24 bg-surface-50 border-b border-slate-200 scroll-mt-20">
      <Container className="max-w-7xl mx-auto px-4 md:px-6">
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          <span className="section-label section-label-primary inline-flex">Mekanisme &amp; Prosedur Pendaftaran</span>
          <h2 className="text-3xl md:text-5xl font-extrabold text-slate-900 tracking-tight">
            Tahapan Alur <span className="text-primary">Penerimaan Santri</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {STEPS.map((step, idx) => (
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 sm:p-7 border border-slate-200 shadow-xs hover:shadow-md hover:border-secondary transition-all relative overflow-hidden flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-5">
                  <div className="w-12 h-12 rounded-xl bg-secondary/20 border border-secondary/40 text-primary flex items-center justify-center font-bold">
                    <step.icon className="w-6 h-6" />
                  </div>
                  <span className="text-2xl font-black text-slate-200 tracking-tight">{step.step}</span>
                </div>

                <h3 className="text-lg font-extrabold text-slate-900 mb-2">{step.title}</h3>
                <p className="text-xs sm:text-sm text-slate-500 leading-relaxed font-normal">{step.description}</p>
              </div>
        ...
        <div className="mt-14 text-center">
          <a href="https://spmb.pesantren-alimam.com/daftar" className="btn-primary">
