...
export default function ProcessSection() {
    return (
        <section id="alur" className="section-alt border-y border-gold-200/50">
            ...
            <Container className="relative z-10">
                <div className="text-center max-w-3xl mx-auto mb-16 lg:mb-20">
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill bg-white border border-gold-200 text-maroon-700 text-xs font-bold uppercase tracking-widest mb-6 shadow-sm"
                    >
                ...
                <div className="relative">
                    ...
                    <div className="hidden lg:block absolute top-[60px] left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-gold-200 to-transparent z-0" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 lg:gap-4 overflow-x-hidden md:overflow-visible">
                        {STEPS.map((step, idx) => (
                            ...
                            >
                                {/* Step Number Badge */}
                                <div className="absolute top-0 -right-2 md:right-4 lg:-right-2 w-8 h-8 md:w-9 md:h-9 rounded-[10px] bg-white shadow-md border border-gold-200 flex items-center justify-center z-20 font-black text-maroon-700 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity text-xs md:text-sm">
                                    {idx + 1}
                                </div>
                                ...
                                <div className={`w-20 h-20 md:w-[120px] md:h-[120px] rounded-[1.5rem] md:rounded-[2rem] border-2 flex items-center justify-center mb-5 md:mb-8 shadow-sm transition-all duration-500 group-hover:scale-105 group-hover:shadow-md ${
                                    ...
                                    step.color === 'gold' ? 'bg-white text-gold-800 border-gold-200 hover:border-gold-200 hover:bg-gold-50' :
                                    'bg-cream-50 text-maroon-600 border-cream-100 hover:bg-cream-100'
                                    }`}>
                                ...
                                {idx < STEPS.length - 1 && (
                                    <div className="lg:hidden mt-6 md:mt-8 text-gold-200">
                                        <div className="w-0.5 h-10 md:h-12 bg-gold-200 mx-auto rounded-full" />
                                    </div>
                                )}
                            </motion.div>
                        ))}
