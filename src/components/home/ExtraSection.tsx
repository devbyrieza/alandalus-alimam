...
export default function ExtraSection() {
    return (
        <section id="ekstrakurikuler" className="section-alt relative border-y border-cream-100 overflow-hidden">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
                    {EXTRA_ACTIVITIES.map((item, idx) => (
                        <motion.div
                            ...
                            className="bg-white p-6 rounded-2xl border border-cream-100 flex flex-col items-center justify-center text-center group hover:border-maroon-200 shadow-premium-sm hover:shadow-premium-md transition-all duration-400 relative overflow-hidden"
                        >
                            ...
                            <div className={`w-12 h-12 rounded-[12px] flex items-center justify-center mb-4 shadow-xs group-hover:scale-105 transition-transform duration-400 relative z-10 ${
                                item.accent === 'maroon' ? 'bg-maroon-50 text-maroon-600' :
                            ...
                            <p className="text-[0.65rem] font-black tracking-[0.12em] text-ink-900 uppercase group-hover:text-maroon-800 transition-colors leading-tight relative z-10">
                                {item.name}
                            </p>
                        </motion.div>
                    ))}
