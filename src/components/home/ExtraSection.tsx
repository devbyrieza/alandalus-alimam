...
export default function ExtraSection() {
    return (
        <section id="ekstrakurikuler" className="section-std border-b border-cream-200/50">
            ...
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gold-100 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 opacity-50 pointer-events-none" />
                    ...
                    {EXTRA_ACTIVITIES.map((item, idx) => (
                        ...
                        >
                            ...
                            <div className={`w-14 h-14 md:w-16 md:h-16 rounded-[1.25rem] flex items-center justify-center mb-5 shadow-premium-sm group-hover:shadow-premium-md group-hover:scale-110 transition-all duration-500 relative z-10 ${
                                ...
                                item.color === 'gold' ? 'bg-gold-50 text-maroon-700' :
                                'bg-gold-100/50 text-gold-700'
                                }`}>
                                <item.icon className="w-7 h-7 md:w-8 md:h-8" />
                            </div>
                            
                            <p className="text-[10px] md:text-[11px] font-black tracking-[0.15em] text-ink-950 uppercase group-hover:text-maroon-900 transition-colors leading-tight relative z-10">
                                {item.name}
                            </p>

                            {/* Subtle decorative dot */}
                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-gold-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                        </motion.div>
                    ))}
                </div>
            </Container>
