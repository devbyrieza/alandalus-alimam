...
export default function TeachersSection() {
    return (
        <section id="pengajar" className="section-alt relative border-y border-cream-100 overflow-hidden">
            ...
            <Container className="relative z-10">
                <div className="flex flex-col lg:flex-row gap-14 lg:gap-24 items-start">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white border border-gold-200 text-maroon-700 text-[0.65rem] font-bold uppercase tracking-[0.12em] mb-5 shadow-xs">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4">
                            {[
                                { icon: Globe, label: "Lulusan Luar Negeri", sub: "Timur Tengah, Mesir & Maroko", accent: "maroon" },
                                { icon: Users, label: "Native Speakers", sub: "Timur Tengah", accent: "gold" },
                                { icon: BookOpen, label: "Lulusan Terbaik", sub: "Dalam Negeri & Pondok Unggulan", accent: "cream" },
                                { icon: Award, label: "Dosen & Pakar", sub: "Tenaga Pendidik Profesional", accent: "maroon" }
                            ].map((feature, i) => (
                                <div key={i} className="group flex items-center gap-4 bg-white p-3.5 rounded-2xl border border-cream-100 shadow-premium-sm hover:shadow-premium-md hover:border-maroon-100 transition-all duration-400">
                                    <div className={`w-11 h-11 rounded-[12px] flex items-center justify-center shrink-0 shadow-xs transition-transform duration-400 group-hover:scale-105 ${
                                        feature.accent === 'maroon' ? 'bg-maroon-50 text-maroon-600' :
                                        feature.accent === 'gold' ? 'bg-gold-50 text-gold-700 border border-gold-100' :
                    ...
                    <div className="lg:w-[58%] grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 w-full">
                        {TEACHER_BACKGROUNDS.map((item, idx) => (
                            <motion.div
                                ...
                                className="group relative bg-white p-5 rounded-2xl border border-cream-100 shadow-premium-sm hover:shadow-premium-md hover:border-maroon-100 flex items-start gap-3.5 overflow-hidden transition-all duration-400"
                            >
                                <div className="absolute inset-0 bg-gradient-to-br from-maroon-50/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                                ...
                                <div className="mt-0.5 w-6 h-6 rounded-full bg-gold-50 border border-gold-100 flex items-center justify-center shrink-0 shadow-xs transition-transform duration-400 group-hover:scale-110 group-hover:bg-gold-100">
                                    <CheckCircle2 className="w-3.5 h-3.5 text-maroon-600" strokeWidth={2.5} />
                                </div>
                                <div className="space-y-1 mt-0.5 relative z-10">
                                    <h4 className="font-bold text-ink-900 text-[0.875rem] leading-snug group-hover:text-maroon-800 transition-colors duration-200">
                                        {item.institution}
                                    </h4>
