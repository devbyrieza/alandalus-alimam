...
const BOARD_MEMBERS = [
    ...
    { name: "Ustadz Nurdin Apud Sarbini, Lc, M.Pd",      image: "/images/nurdin-apud-sabrini.webp" },
    { name: "H. Tarmen Tascha, SE",                        image: "/images/tarmen-tascha.webp" },
    { name: "Ustadz Aminullah Yasin, Lc, M.Pd",           image: "/images/aminullah-yasin.webp" },
...
function MemberCard({ name, image, index }: { name: string; image: string; index: number }) {
    return (
        ...
        >
            ...
            {/* Chevron hint */}
            <Star
                className="w-3.5 h-3.5 shrink-0 text-gold-300 fill-gold-300 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                aria-hidden
            />
        </motion.div>
    );
    ...
    return (
        <section id="pembina" className="section-std relative overflow-hidden">
            ...
            <div
                ...
                style={{ background: "radial-gradient(circle, rgba(253,242,242,0.55) 0%, transparent 65%)" }}
            />
            <div
                className="absolute -bottom-24 -left-24 w-[360px] h-[360px] pointer-events-none"
                style={{ background: "radial-gradient(circle, rgba(254,243,199,0.3) 0%, transparent 65%)" }}
            />
