    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled
            ? "glass-panel border-b border-[var(--color-maroon-100)] py-2"
          ...
          <div className="flex items-center justify-between">
            ...
            <nav
              className={`hidden lg:flex items-center gap-0.5 p-1.5 rounded-full border transition-all duration-300 ${
                isScrolled
                  ? "bg-white/80 border-[var(--color-maroon-100)] shadow-sm"
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`px-4 py-2 text-sm font-bold rounded-full transition-all duration-300 min-h-[40px] flex items-center ${
                    isActive(link.href)
                      ? "bg-[var(--color-maroon-800)] text-[var(--color-cream-100)] shadow-[var(--shadow-maroon)]"
                      : "text-[var(--color-ink-600)] hover:text-[var(--color-maroon-800)] hover:bg-[var(--color-maroon-50)]"
                  }`}
                >
                  {link.label}
                </Link>
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <Link
                href="/login"
                className="text-sm font-bold text-[var(--color-ink-600)] hover:text-[var(--color-maroon-800)] transition-colors duration-200 px-4 py-2 min-h-[40px] flex items-center rounded-full hover:bg-[var(--color-maroon-50)]"
              >
                Masuk
              </Link>
              <Link
                href="/ppdb"
                className="btn-primary flex items-center gap-2 group text-sm px-5 py-2.5 glow-ring-maroon"
              >
                Daftar PPDB
            ...
            <button
              ...
              className={`lg:hidden p-3 rounded-2xl transition-all duration-300 min-h-[48px] min-w-[48px] flex items-center justify-center border ${
                isScrolled
                  ? "bg-white border-[var(--color-maroon-100)] text-[var(--color-maroon-700)] hover:bg-[var(--color-maroon-50)] shadow-[var(--shadow-xs)]"
            ...
            >
              ...
              <div className="flex items-center gap-3 px-6 py-3 border-b border-[var(--color-maroon-50)] flex-shrink-0">
                ...
                <div className="ml-auto">
                  <span className="text-[10px] font-black px-2.5 py-1 rounded-full bg-[var(--color-maroon-50)] text-[var(--color-maroon-600)] border border-[var(--color-maroon-100)] uppercase tracking-wider">
                    Template
                  </span>
                </div>
              </div>
              ...
              <div className="overflow-y-auto flex-1 px-4 py-4 pb-safe">
                    ...
                    >
                      <Link
                        ...
                        className={`px-4 py-4 rounded-xl text-base font-bold transition-all min-h-[56px] flex items-center justify-between group ${
                          isActive(link.href)
                            ? "bg-[var(--color-maroon-800)] text-[var(--color-cream-100)] shadow-[var(--shadow-maroon)]"
                            : "text-[var(--color-ink-800)] hover:bg-[var(--color-maroon-50)] hover:text-[var(--color-maroon-800)]"
                        }`}
                      >
                        <span>{link.label}</span>
                        {isActive(link.href) && (
                          <div className="w-1.5 h-1.5 rounded-full bg-[var(--color-cream-300)]/70" />
                        )}
                      </Link>
                    </motion.div>
                  ))}
                ...
                <div className="flex flex-col gap-3">
                  <Link
                    href="/login"
                    onClick={() => setIsMenuOpen(false)}
                    className="w-full py-4 text-sm font-bold rounded-xl border border-[var(--color-maroon-100)] text-[var(--color-maroon-700)] bg-[var(--color-maroon-50)] hover:bg-[var(--color-maroon-100)] text-center transition-all min-h-[52px] flex items-center justify-center gap-2"
                  >
                    Masuk ke Dashboard
                  </Link>
                  <Link
