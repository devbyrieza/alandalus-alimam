...
const FeatureCard = ({
  ...
  >
    ...
    <div className={`relative ${index % 2 === 1 ? 'lg:col-start-2' : ''}`}>
      <div className={`aspect-4/3 rounded-[3.5rem] overflow-hidden shadow-lg relative group ${index % 2 === 0 ? 'rotate-2' : '-rotate-2'} hover:rotate-0 transition-transform duration-700 border border-maroon-100 p-2 bg-white`}>
      ...
      {/* Program Badge */}
      <div className="absolute -top-4 -right-4 bg-white px-6 py-3 rounded-pill shadow-md border border-maroon-100 z-10 flex items-center gap-2">
    ...
    <div className="flex flex-col justify-center">
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        {item.stats.map((stat, statIdx) => (
          <div
            key={statIdx}
            className="bg-maroon-50 p-4 rounded-[1.5rem] border border-maroon-100 text-center hover:bg-white hover:shadow-sm transition-all"
          >
            <div className="text-2xl font-black text-maroon-700 mb-1">
              {stat.value}
            </div>
      ...
      {/* Results List */}
      <div className="app-card bg-white p-8 rounded-[2.5rem] border border-maroon-100 shadow-sm mb-8">
        <div className="space-y-4">
          {item.results.map((result, idx) => (
            <div key={idx} className="flex items-start gap-4">
              <result.icon className="w-5 h-5 text-maroon-600 mt-0.5 shrink-0" />
...
const StatsCard = ({ icon: Icon, value, label, sublabel }: HeroStat) => (
   <motion.div
    whileHover={{ y: -5 }}
    className="app-card bg-white p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border border-maroon-100 shadow-sm text-center min-w-[140px] md:min-w-[160px] flex flex-col items-center"
  >
    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-maroon-50 flex items-center justify-center text-maroon-600 mb-3 md:mb-4 border border-maroon-100">
...
const ScheduleCard = ({
  ...
  index: number;
   ...
   <motion.div
    ...
    className={`group p-10 rounded-[3rem] ${schedule.bgColor} border border-maroon-100 shadow-sm hover:shadow-md transition-all duration-500 hover:-translate-y-2`}
  >
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-8">
        <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform border border-maroon-100">
          <schedule.icon className={`${schedule.iconColor} w-8 h-8`} />
        </div>
        <div className="text-lg font-black text-maroon-700 bg-white/50 px-4 py-1.5 rounded-pill border border-maroon-100">
          {schedule.time}
        </div>
      </div>
      ...
      <p className="text-base md:text-lg text-ink-600 mb-6 md:mb-8 font-medium leading-relaxed">
       ...
       <div className="mt-auto inline-flex items-center gap-3 bg-white/60 p-4 rounded-2xl border border-maroon-100 shadow-sm">
        <Zap className="w-5 h-5 text-maroon-600 flex-shrink-0" />
        <span className="text-sm font-black text-ink-950 uppercase tracking-tight">
          {schedule.benefit}
        </span>
      </div>
...
const WeeklyActivityCard = ({
  ...
  index: number;
   ...
   <motion.div
    ...
    className={`app-card group p-8 rounded-[2.5rem] bg-white border border-maroon-50 shadow-sm hover:shadow-md transition-all hover:-translate-y-2 text-center`}
  >
    <div className={`w-14 h-14 ${activity.bg} rounded-2xl flex items-center justify-center ${activity.color} mx-auto mb-6 group-hover:scale-110 transition-transform border border-maroon-100`}>
...
const EkskulCard = ({
  ...
  index: number;
   ...
   <motion.div
    ...
    viewport={{ once: true }}
    transition={{ delay: index * 0.05 }}
    className="app-card group p-4 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-white border border-maroon-50 shadow-sm hover:shadow-md hover:border-maroon-200 transition-all hover:-translate-y-2 text-center h-full flex flex-col justify-between"
  >
    <div className="w-12 h-12 md:w-16 md:h-16 bg-maroon-50 rounded-xl md:rounded-2xl flex items-center justify-center text-maroon-600 mx-auto mb-4 md:mb-6 group-hover:scale-110 transition-all duration-300 border border-maroon-100">
      ...
      <section className="relative py-24 md:py-32 overflow-hidden bg-white">
        ...
        <Container className="relative z-10 text-center">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill bg-maroon-50 border border-maroon-100 text-maroon-700 text-xs font-bold uppercase tracking-widest mb-10 shadow-sm"
          >
          ...
          >
            ...
            <div className="relative z-10 max-w-4xl mx-auto">
              ...
              <h2 className="text-3xl md:text-5xl lg:text-6xl font-display font-black text-ink-950 mb-6 md:mb-8 leading-tight">
                Hasil Nyata <br />
                <span className="text-maroon-600">Pendidikan Kita</span>
              </h2>
              ...
              <div className="flex justify-center">
                <Link href="/kontak">
                  <button className="px-12 py-3 md:px-14 md:py-4 rounded-pill bg-maroon-900 text-white font-black text-base md:text-lg hover:bg-gold-500 shadow-premium-lg transition-all flex items-center gap-3">
            ...
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 w-full">
              {[
                ...
                <motion.div
                  ...
                  className="bg-white p-5 md:p-6 rounded-[2rem] flex items-center gap-4 border border-maroon-100 group transition-all h-full text-left"
                >
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-maroon-50 shadow-sm flex items-center justify-center text-maroon-600 shrink-0 group-hover:bg-maroon-600 group-hover:text-white transition-colors border border-maroon-100">
      ...
      <section className="py-24 md:py-32">
        <Container>
          <div className="text-center max-w-3xl mx-auto mb-20">
            ...
            <p className="text-xl text-ink-600 font-medium leading-relaxed">
              Aktivitas rutin pekanan dan bulanan yang dirancang untuk mematangkan <span className="text-maroon-600 font-bold uppercase tracking-tight">keterampilan non-teknis</span> dan pengalaman santri.
            </p>
          </div>
      ...
      <section className="py-16 md:py-24 lg:py-32 bg-white">
        <Container>
          <motion.div
            ...
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            className="bg-maroon-900 bg-linear-to-br from-maroon-800 to-maroon-950 rounded-[2rem] md:rounded-[3rem] lg:rounded-[4rem] p-6 sm:p-8 md:p-16 lg:p-24 text-center text-white relative overflow-hidden shadow-lg"
          >
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-6 justify-center px-4">
                <Link href="/ppdb">
                  <button className="w-full sm:w-auto px-10 py-3.5 sm:py-4 md:py-5 rounded-pill bg-white text-maroon-900 font-black text-sm sm:text-base md:text-lg hover:bg-maroon-50 shadow-md transition-all min-h-[48px] sm:min-h-[52px]">
                    Daftar Sekarang
                  </button>
                </Link>
                <Link href="/kontak">
