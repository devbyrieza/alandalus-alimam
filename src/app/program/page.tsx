...
const PROGRAMS = [
    ...
    },
    {
        ...
        description: "Program persiapan intensif menuju jenjang Aliyah, berfokus pada pemantapan Bahasa Arab, Tahfidz, dan kematangan kepemimpinan organisasi untuk mencetak kader ulama yang kontributif.",
        stats: [
                        ...
                        {PROGRAMS.map((program) => (
                            <button
                                ...
                                ${activeSection === program.id
                                        ...
                                        : 'bg-white text-ink-500 border-maroon-100 hover:border-maroon-300 hover:text-maroon-700'
                                    }`}
                            >
                                {program.name}
                            </button>
                    ...
                    <section key={program.id} id={program.id} className="py-24 md:py-32 scroll-mt-32 overflow-hidden">
                        <Container>
                            <div className={`grid lg:grid-cols-2 gap-16 lg:gap-24 items-center ${idx % 2 === 1 ? 'lg:grid-flow-dense' : ''}`}>
                                ...
                                >
                                    <div className="aspect-4/5 rounded-[3.5rem] overflow-hidden shadow-lg relative z-10 p-3 bg-white border border-maroon-100">
                                    ...
                                    <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] rounded-full blur-[120px] -z-10 opacity-30
                                        ${program.theme === 'maroon' ? 'bg-maroon-100' : 'bg-cream-200'}
                                    `} />
                                </motion.div>
                                ...
                                <div className={idx % 2 === 1 ? 'lg:col-start-1' : ''}>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                                        {program.stats.map((stat, sIdx) => (
                                             <div key={sIdx} className={`app-card bg-white p-5 rounded-[2rem] border border-maroon-50 shadow-sm hover:shadow-md transition-all text-center group`}
                                            >
                                                <div className={`w-10 h-10 rounded-xl mx-auto mb-3 flex items-center justify-center transition-transform group-hover:scale-110 border border-maroon-100
                                                    ${program.theme === 'maroon' ? 'bg-maroon-50 text-maroon-600' : 'bg-cream-100 text-gold-700'}
                                                `}>
                                    ...
                                    <motion.div
                                        ...
                                        whileInView={{ opacity: 1, y: 0 }}
                                        viewport={{ once: true }}
                                        className={`app-card rounded-[3rem] p-10 mb-10 border ${program.bg} border-maroon-100 shadow-sm relative overflow-hidden`}
                                    >
                                        <div className="absolute top-0 right-0 p-8 opacity-5">
                                            <BookOpen className="w-32 h-32 text-maroon-900" />
                                        </div>

                                        <h3 className="text-2xl font-black text-ink-950 mb-8 flex items-center gap-3">
                                            <div className={`w-3 h-10 rounded-full ${program.theme === 'maroon' ? 'bg-maroon-600' : 'bg-gold-500'}`} />
                                            Kurikulum & Fokus
                                        </h3>
                                            ...
                                            {program.curriculum.map((item, cIdx) => (
                                                <li key={cIdx} className="flex items-start gap-4 group/item">
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 shadow-sm
                                                        ${program.theme === 'maroon' ? 'bg-maroon-600 text-white' : 'bg-gold-500 text-white'}
                                                    `}>
                                    ...
                                    >
                                        <Link href={`/daftar?program=${program.id}`}>
                                            <button className={`w-full sm:w-auto px-14 py-5 rounded-pill font-black text-white text-lg shadow-md transition-all hover:-translate-y-1
                                                ${program.theme === 'maroon' ? 'bg-maroon-700 hover:bg-maroon-800' : 'bg-gold-600 hover:bg-gold-700'}
                                            `}>
                                                Daftarkan Sekarang
                                            </button>
                                        </Link>
            ...
            <section className="py-16 md:py-24 lg:py-32 bg-surface-50">
                <Container>
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className="bg-maroon-900 bg-linear-to-br from-maroon-800 to-maroon-950 rounded-[2rem] md:rounded-[3rem] lg:rounded-[4rem] p-6 sm:p-8 md:p-16 lg:p-24 text-center text-white relative overflow-hidden"
                    >
                        ...
                        <div className="relative z-10">
                            <h2 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-display font-black mb-4 sm:mb-6 md:mb-8 text-white leading-tight">
                                Mulai Perjalanan <br /> <span className="text-cream-300">Terbaik</span> Mereka
                            </h2>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-6 justify-center px-4">
                                <Link href="/daftar">
                                    <button className="w-full sm:w-auto px-6 sm:px-10 py-3.5 sm:py-4 md:py-5 rounded-pill bg-white text-maroon-900 font-black text-sm sm:text-base md:text-lg hover:bg-maroon-50 shadow-md transition-all min-h-[48px] sm:min-h-[52px]">
                                        Pendaftaran Online
                                    </button>
                                </Link>
                                <Link href="/kontak">
