...
export async function GET() {
  try {
    ...
    if (!pendaftar || !pendaftar.dompet) {
      return NextResponse.json({ success: false, message: "Dompet ZAD tidak ditemukan" });
    }

    const transaksiList = pendaftar.dompet.transaksi.map(tx => ({
      id: tx.id,
    ...
    return NextResponse.json({
      ...
      data: {
        dompet: {
          saldo: Number(pendaftar.dompet.saldo),
          status: pendaftar.dompet.status,
          is_limit_terbuka: pendaftar.dompet.is_limit_terbuka
        },
        transaksiList,
        tagihanList
      }
