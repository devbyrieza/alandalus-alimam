      ...
      <div className="px-3 py-1">
        <Link
          ...
          className={`group flex items-center px-4 py-3.5 text-sm font-bold rounded-2xl transition-all duration-200 ${item.active
            ...
            : "text-ink-600 hover:bg-gold-100 hover:text-maroon-900"
            }`}
        >
          <item.icon className={`w-5 h-5 mr-3 flex-shrink-0 transition-colors ${item.active ? 'text-gold-200' : 'text-ink-400 group-hover:text-maroon-700'}`} />
          <span className="flex-1 truncate">{item.name}</span>
    ...
    return (
      <div className="min-h-screen bg-surface-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-3xl shadow-clay-lg text-center max-w-sm w-full mx-4">
          ...
          <h1 className="text-3xl font-serif font-black uppercase text-maroon-950 mb-2">Pondok Pesantren {BRANDING.schoolName}</h1>
      ...
      <div className="min-h-screen bg-surface-50 font-sans selection:bg-maroon-100 selection:text-maroon-900">
        ...
        <div className="lg:hidden bg-white/90 backdrop-blur-xl sticky top-0 z-40 px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between border-b border-gold-200 shadow-sm">
          <div className="flex items-center gap-2 sm:gap-3">
            <button onClick={() => setSidebarOpen(true)} className="p-2 -ml-2 text-maroon-950 hover:bg-gold-50 rounded-xl transition-colors">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm sm:text-base font-black text-maroon-950 leading-none">Portal Santri</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-linear-to-br from-maroon-700 to-maroon-900 flex items-center justify-center text-white text-sm font-black shadow-md border border-gold-100">
            ...
            <div className="flex flex-col h-full bg-white/70 backdrop-blur-xl border-r border-white/50 shadow-clay-lg">
              ...
              <div className="px-6 pt-8 pb-6 border-b border-gold-100/50 mb-2">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h1 className="font-black text-xl text-maroon-950 leading-none tracking-tight">PPDB <span className="text-maroon-700">{BRANDING.schoolName}</span></h1>
              ...
              <div className="flex flex-col h-full overflow-hidden">
                <div className="p-6 border-b border-surface-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-lg text-maroon-950 tracking-tight">PPDB {BRANDING.schoolName}</span>
                  </div>
                  <button onClick={() => setSidebarOpen(false)} className="p-2 text-ink-400 hover:text-maroon-700 bg-gold-50 rounded-full">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="p-5 bg-gold-50 border-b border-gold-100">
                  ...
                  <p className="font-mono text-xs font-bold text-maroon-700 bg-white px-2 py-1 rounded-lg inline-block border border-gold-100">{nomorPendaftaran}</p>
                </div>
          ...
          <main className="flex-1 lg:pl-72 w-full transition-all duration-300 flex flex-col min-h-screen relative pb-24 lg:pb-0">
            ...
            <header className="hidden lg:flex sticky top-4 z-30 mx-8 mt-4 rounded-[1.5rem] bg-white/70 backdrop-blur-xl px-6 py-4 items-center justify-between shadow-sm border border-gold-100">
              ...
              <div className="flex items-center gap-4">
                <Link href="/" className="p-2 text-ink-400 hover:text-maroon-700 bg-gold-50 hover:bg-gold-100 rounded-full transition-colors" title="Ke Beranda Website">
              ...
              <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gold-200 pb-safe z-40 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.1)] rounded-t-[1.5rem]">
                <div className="flex justify-around items-center px-4 py-3">
                  <Link href="/dashboard/pendaftar" className="flex flex-col items-center p-2 group w-16">
                    <div className={`w-10 h-8 rounded-full flex items-center justify-center mb-1 transition-colors ${pathname === '/dashboard/pendaftar' ? 'bg-gold-100' : 'bg-transparent group-hover:bg-gold-50'}`}>
                  ...
                  <Link href="/dashboard/pendaftar/pembayaran-pendaftaran" className="flex flex-col items-center p-2 group w-16">
                    <div className={`w-10 h-8 rounded-full flex items-center justify-center mb-1 transition-colors ${pathname.includes('pembayaran') ? 'bg-gold-100' : 'bg-transparent group-hover:bg-gold-50'}`}>
                  ...
                  <Link href="/dashboard/pendaftar/isi-data-lengkap" className="flex flex-col items-center p-2 group w-16">
                    <div className={`w-10 h-8 rounded-full flex items-center justify-center mb-1 transition-colors ${pathname.includes('isi-data-lengkap') || pathname.includes('upload-berkas') ? 'bg-gold-100' : 'bg-transparent group-hover:bg-gold-50'}`}>
                      <ClipboardList className={`w-5 h-5 transition-colors ${pathname.includes('isi-data-lengkap') || pathname.includes('upload-berkas') ? 'text-maroon-700' : 'text-ink-400 group-hover:text-maroon-600'}`} />
                    </div>
                    <span className={`text-[10px] font-bold text-center ${pathname.includes('isi-data-lengkap') || pathname.includes('upload-berkas') ? 'text-maroon-800' : 'text-ink-400'}`}>Data</span>
                  </Link>
