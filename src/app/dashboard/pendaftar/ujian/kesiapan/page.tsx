...
export default function KesiapanTestPage() {
    ...
    const handleSubmit = async () => {
        if (Object.keys(answers).length < totalQuestions) {
            Swal.fire('Perhatian', 'Mohon lengkapi semua jawaban sebelum mengirim.', 'warning');
            return;
        }
        ...
        try {
            ...
            await Swal.fire({ icon: 'success', title: 'Alhamdulillah!', text: 'Tes Kesiapan berhasil diselesaikan.', confirmButtonColor: '#059669' });
    ...
    if (checking) {
        return <div className="flex items-center justify-center min-h-[400px]"><Loader2 className="w-10 h-10 animate-spin text-teal-600" /></div>;
    }
        ...
        return (
            <div className="max-w-lg mx-auto p-8 text-center mt-10">
                <div className="bg-white rounded-2xl shadow-lg border p-10">
                    ...
                    <p className="text-stone-600 mb-6">Anda sudah menyelesaikan Tes Kesiapan sebelumnya.</p>
                    <button onClick={() => router.push('/dashboard/pendaftar?tab=undangan-seleksi')} className="px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl transition-colors">
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 md:p-6 pb-24">
            ...
            <div className="bg-gradient-to-r from-maroon-600 to-maroon-800 rounded-xl p-6 mb-6">
                <h1 className="text-xl font-bold text-white">Tes Kesiapan Calon Santri/Wati</h1>
                <p className="text-maroon-100 text-sm mt-1">15 pernyataan • Skala 1-5 • Durasi 45 menit</p>
            </div>
                        ...
                        {section.items.map((q) => (
                            <div key={q.id} className="border-b pb-6 last:border-0 last:pb-0">
                                ...
                                <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                                    {[1, 2, 3, 4, 5].map(val => (
                                        <label key={val} className={`cursor-pointer border p-3 rounded-lg flex-1 text-center hover:bg-slate-50 transition-all ${answers[q.id] === val ? 'bg-emerald-100 border-emerald-500 font-bold text-emerald-700 shadow-sm' : 'border-stone-200'}`}>
