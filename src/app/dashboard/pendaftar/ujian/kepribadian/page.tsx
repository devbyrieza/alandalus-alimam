      ...
      <div className="max-w-lg mx-auto p-8 text-center mt-10">
        <div className="bg-white rounded-2xl shadow-lg border p-10">
          ...
          <button
            ...
            className="px-6 py-3 bg-maroon-600 hover:bg-maroon-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-maroon-100"
          >
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-24">
      ...
      <div className="bg-maroon-50 border border-maroon-200 rounded-xl p-4">
        <div className="flex gap-4">
          ...
          <div>
            <h4 className="font-bold text-maroon-800 mb-1 text-sm">
              Pesan dari Mudir
            </h4>
      ...
      <div className="bg-white rounded-xl border p-4 mb-6 shadow-sm mt-6">
        <div className="flex justify-between items-center mb-2 text-sm">
          ...
          <span className="font-bold text-maroon-600">
            {Object.keys(answers).length}/{KEPRIBADIAN_QUESTIONS.length} dijawab
          </span>
        </div>
        <div className="w-full h-2 bg-stone-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-linear-to-r from-maroon-500 to-maroon-700 rounded-full transition-all duration-500 shadow-sm shadow-maroon-200"
            style={{
        ...
        {currentQuestions.map((q) => (
          <div key={q.id} className="bg-white p-5 rounded-xl border shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              >
                <span
                  className={`font-black mr-3 border rounded-lg px-2.5 py-1 text-xs transition-colors ${answers[q.id] === "A" ? "bg-maroon-600 text-white border-maroon-600" : "bg-white text-stone-400 border-stone-200"}`}
                >
                  A
                </span>
              ...
              >
                <span
                  className={`font-black mr-3 border rounded-lg px-2.5 py-1 text-xs transition-colors ${answers[q.id] === "B" ? "bg-maroon-600 text-white border-maroon-600" : "bg-white text-stone-400 border-stone-200"}`}
                >
                  B
                </span>
                <span className="font-bold text-sm sm:text-base">
