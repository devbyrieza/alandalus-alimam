...
export default function DaftarUlangTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (amount < 4250000 && !keringananReason.trim()) {
      setMessage({
        type: "error",
    ...
    if (numericNominal < 8500000) formData.append("cicilan_ke", cicilanKe);
            ...
            <div className="pt-2 border-t border-slate-100 animate-in fade-in slide-in-from-top-1">
              ...
              <div className="relative w-32">
                <input
                  ...
                  className="w-full px-4 py-3 text-lg font-black text-ink-900 border border-ink-300 rounded-xl focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 transition-all shadow-inner"
                />
              </div>
            </div>
          )}
            ...
            {numericNominal > 0 && tipeBayar === "CICILAN DIBAWAH 50%" && (
              <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl space-y-3 animate-in fade-in slide-in-from-top-1">
                ...
                <textarea
                  ...
                  className="w-full p-3 text-xs border border-amber-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all bg-white font-medium min-h-[80px]"
                  required
                />
              </div>
            )}
