...
export default function ProgressTracker({ currentStatus }: ProgressTrackerProps) {
  ...
  return (
    <div className="bg-white rounded-3xl md:rounded-4xl p-6 sm:p-8 border border-gold-100 shadow-clay-lg overflow-hidden relative group">
      {/* Abstract Background Element */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-gold-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 opacity-50 group-hover:scale-110 transition-transform duration-700" />
      
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-8 md:mb-12">
          <div>
            <h3 className="text-xl md:text-2xl font-black text-maroon-950 tracking-tight leading-none mb-2">Alur Pendaftaran</h3>
            <p className="text-[10px] uppercase font-black tracking-widest text-ink-400">Status Saat Ini: <span className="text-maroon-700">{statusInfo.label}</span></p>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-gold-50 flex items-center justify-center border border-gold-100 shadow-clay-sm">
              ...
              <div key={phase.id} className="flex gap-4 md:gap-6 group/item relative">
                ...
                {idx < phases.length - 1 && (
                  <div className={`absolute top-10 left-5 md:left-6 w-0.5 h-full -ml-px bg-stone-100 z-0`}>
                    <div className={`h-full bg-maroon-600 transition-all duration-1000 ${isCompleted ? 'scale-y-100' : 'scale-y-0'} origin-top`} />
                  </div>
                )}
                ...
                <div 
                  className={`w-10 h-10 md:w-12 md:h-12 rounded-2xl flex items-center justify-center relative z-10 transition-all duration-500 border-2 ${
                    isCompleted 
                      ? "bg-maroon-700 border-maroon-700 text-white shadow-maroon-200 shadow-xl" 
                      : isCurrent 
                        ? "bg-white border-maroon-700 text-maroon-700 shadow-maroon-100 shadow-xl scale-110" 
                ...
                <div className="flex-1 pt-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className={`text-sm md:text-base font-black tracking-tight transition-colors ${
                      isCompleted ? "text-maroon-900" : isCurrent ? "text-maroon-800" : "text-stone-400"
                    }`}>
                      {phase.label}
                    </h4>
                    {isCurrent && (
                      <span className="flex items-center gap-1 text-[8px] uppercase font-black tracking-widest px-2 py-0.5 bg-gold-400 text-maroon-950 rounded-full animate-pulse">
                        Proses
                      </span>
                    )}
                  </div>
                ...
                {isCurrent && (
                  <div className="hidden md:flex items-center">
                    <div className="p-2 bg-maroon-50 rounded-full text-maroon-700 group-hover/item:translate-x-1 transition-transform">
