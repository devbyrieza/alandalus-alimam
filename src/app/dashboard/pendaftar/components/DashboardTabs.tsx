...
export default function DashboardTabs({ statusProses }: DashboardTabsProps) {
  ...
  const tabs: { id: TabName; label: string; icon: any; href: string }[] = [
    ...
    { id: "daftar-ulang", label: "Daftar Ulang", icon: CheckCircle, href: "/dashboard/pendaftar/daftar-ulang" },
    { id: "profil", label: "Profil", icon: Settings, href: "/dashboard/pendaftar/profil" },
  ];
      ...
      <div className="max-w-7xl mx-auto flex items-center overflow-x-auto scrollbar-hide px-4 py-2 gap-2 sm:gap-4 no-scrollbar">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform group-hover:scale-110 ${isActive ? 'text-gold-300' : ''}`} />
