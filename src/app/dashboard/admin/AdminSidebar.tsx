...
export default function AdminSidebar({ children, userRole, adminName, userId, availableRoles }: AdminSidebarProps) {
        ...
        return (
            <Link
                ...
                className={`nav-link group flex items-center gap-3 px-3 py-2 rounded-xl transition-all duration-200 ${item.active
                    ? "active bg-white text-maroon-700 shadow-clay-sm border border-maroon-100"
                    : "text-ink-500 hover:bg-gold-50 hover:text-ink-900"
                    }`}
            >
                <item.icon className={`w-4 h-4 transition-colors ${item.active ? 'text-maroon-600' : 'text-ink-400 group-hover:text-ink-600'}`} />
    ...
    return (
        <div className="min-h-screen font-sans bg-gold-50/30">
            ...
            <div className="lg:hidden fixed top-0 left-0 right-0 z-[60] bg-white/80 backdrop-blur-xl border-b border-white/20 shadow-sm px-5 py-4 flex items-center justify-between">
                <button
                    ...
                    className="p-2.5 -ml-2 rounded-2xl text-ink-600 hover:bg-gold-100 active:scale-95 transition-all"
                >
                    <Menu className="w-6 h-6" />
                </button>

                <div className="flex flex-col items-center">
                    <span className="font-black text-maroon-900 tracking-tight leading-none text-base">{BRANDING.schoolShortName}</span>
                    <span className="text-[9px] font-black text-maroon-600/60 uppercase tracking-widest mt-1">Admin Panel</span>
                </div>

                <div className="w-9 h-9 rounded-2xl bg-linear-to-br from-maroon-700 to-maroon-900 flex items-center justify-center text-white text-sm font-black shadow-md border border-gold-100">
                    {adminName.charAt(0)}
                </div>
            </div>
                            ...
                            {availableRoles && availableRoles.length > 1 && (
                                <div className="mb-3">
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-ink-50 border border-ink-200 text-sm text-ink-900 font-black rounded-xl py-2 px-3 focus:ring-2 focus:ring-maroon-600/20"
                                    >
                    ...
                    <div className="flex flex-col h-full bg-white/70 backdrop-blur-xl border-r border-white/50 shadow-clay-lg">
                        ...
                        <div className="px-6 py-8 pb-4">
                            <div className="flex items-center gap-3 mb-8">
                                <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-xl shadow-maroon-900/10 ring-4 ring-gold-50/50 overflow-hidden">
                                    <Image 
                                        src={BRANDING.logoPath} 
                                        alt="Logo" 
                                        width={40}
                                ...
                                <div>
                                    <h2 className="font-black text-lg text-maroon-950 leading-tight">Panel <span className="text-maroon-700">Admin</span></h2>
                        ...
                        <div className="p-4 border-t border-ink-100/50 bg-white/50 backdrop-blur-sm">
                            {availableRoles && availableRoles.length > 1 && (
                                <div className="mb-3 px-2">
                                    ...
                                    <select
                                        value={userRole || ""}
                                        onChange={handleRoleSwitch}
                                        className="w-full bg-gold-100 border-none text-xs text-maroon-900 font-black rounded-lg py-1.5 focus:ring-2 focus:ring-maroon-600/20 cursor-pointer"
                                    >
                                        {availableRoles.map(role => (
                                            <option key={role} value={role}>{ROLE_LABELS[role as UserRole] || role}</option>
                                        ))}
                            ...
                            <div className="flex items-center gap-3 p-2.5 rounded-2xl hover:bg-white transition-all cursor-pointer group hover:shadow-clay-sm border border-transparent hover:border-gold-100">
                                <div className="w-10 h-10 rounded-full bg-gold-100 flex items-center justify-center text-maroon-900 font-black group-hover:bg-maroon-50 group-hover:text-maroon-700 transition-colors border border-gold-200">
                                    {adminName.charAt(0)}
                                </div>
                    ...
                    <header className="hidden lg:flex sticky top-4 z-30 mx-8 mt-4 rounded-2xl glass px-6 py-3 items-center justify-between shadow-clay-sm border border-white/40">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-3 pl-4 border-l border-ink-100">
                                <Link href="/" className="text-sm font-black text-maroon-700 hover:underline">Lihat Website</Link>
                            </div>
                        </div>
                    </header>
