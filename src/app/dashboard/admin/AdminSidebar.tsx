...
export default function AdminSidebar({ children, userRole, adminName, userId, availableRoles }: AdminSidebarProps) {
    ...
    const handleLogout = async () => {
        const result = await Swal.fire({
            ...
            confirmButtonColor: '#7c2d12', // Maroon dark
    ...
    return (
        <div className="min-h-screen bg-[#fafafa] font-sans selection:bg-maroon-100 selection:text-maroon-900">
            {/* Mobile Header */}
            <div className="lg:hidden fixed top-0 left-0 right-0 z-[60] bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-5 py-3 flex items-center justify-between">
                ...
                <div className="w-8 h-8 rounded-full bg-maroon-700 flex items-center justify-center text-white text-xs font-bold">
                    {adminName.charAt(0)}
                </div>
            </div>
                            ...
                            <nav className="flex-1 overflow-y-auto p-4 space-y-1">
                                {menuItems.map((item, idx) => (
                                    <Link 
                                        ...
                                        href={item.href} 
                                        onClick={() => setSidebarOpen(false)}
                                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${item.active ? "bg-maroon-50 text-maroon-700 font-semibold" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
                                    >
                                        <item.icon className={`w-4 h-4 ${item.active ? "text-maroon-600" : "text-slate-400"}`} />
                        ...
                        <div className="px-3 mb-4">
                            <div className="relative group">
                                <select
                                    ...
                                    className="w-full appearance-none bg-slate-50 border border-slate-200 text-[11px] font-bold text-slate-600 uppercase tracking-wider px-3 py-2 rounded-lg cursor-pointer focus:outline-none focus:ring-2 focus:ring-maroon-500/10 transition-all group-hover:border-slate-300"
                                >
                                    ...
                                    >
                                        {item.active && (
                                            <motion.div 
                                                ...
                                                className="absolute inset-0 bg-maroon-50/60 rounded-lg -z-10 border border-maroon-100/50"
                                                transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                                            />
                                        )}
                                        <item.icon className={`w-[18px] h-[18px] shrink-0 transition-colors ${item.active ? "text-maroon-600" : "text-slate-400 group-hover:text-slate-600"}`} />
                ...
                <header className="hidden lg:flex sticky top-0 z-40 bg-white/60 backdrop-blur-md border-b border-slate-200/60 h-16 items-center justify-between px-8">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="relative group">
                            ...
                            <input 
                                ...
                                className="bg-slate-100/50 border-none rounded-lg pl-9 pr-4 py-1.5 text-[13px] w-64 focus:w-80 focus:ring-2 focus:ring-maroon-500/10 transition-all placeholder:text-slate-400"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <button className="relative p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-full transition-all">
                            ...
                            <span className="absolute top-2 right-2 w-2 h-2 bg-maroon-500 rounded-full border-2 border-white shadow-sm" />
                        </button>
                        <Link href="/" className="px-4 py-2 bg-maroon-900 text-white text-[11px] font-bold uppercase tracking-widest rounded-lg hover:bg-maroon-800 transition-all shadow-lg shadow-maroon-950/10">
                            Lihat Website
                        </Link>
                    </div>
                </header>
