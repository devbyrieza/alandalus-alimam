...
export default function BantuanBiayaPage() {
  ...
  return (
    <div className="p-6 sm:p-8 md:p-8 space-y-6 max-w-[1200px] mx-auto">
      {/* ── Header ── */}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center bg-white rounded-3xl p-1 border border-stone-200 shadow-2xl shadow-primary/30  shadow-primary/20  shadow-primary/10 overflow-x-auto max-w-full">
          {(["SEMUA", "BEASISWA", "KERINGANAN"] as FilterTab[]).map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-2xl transition-all whitespace-nowrap ${
                filterTab === t
                  ? t === "BEASISWA"
        ...
        <div className="relative w-full lg:w-auto lg:min-w-[260px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-white border border-stone-200 rounded-3xl text-sm font-medium focus:border-primary-400 focus:ring-2 focus:ring-primary-400/20 outline-none"
          />
        </div>
      </div>
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-stone-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {isBeasiswa ? (
                          <span className="flex items-center gap-1.5 bg-emerald-50 text-[#550000] border border-emerald-200 px-3 py-1.5 rounded-full text-xs font-black w-max">
                      ...
                      <td className="px-6 py-4">
                        {pUP > 0 ? (
                          <span className={`font-bold ${isBeasiswa ? "text-[#550000]" : "text-amber-700"}`}>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-2xl w-max ${
                            item.status === "DISETUJUI"
                              ? "bg-green-50 text-[#550000] border border-green-100"
                              : item.status === "DITOLAK"
