...
function StatusBadge({ status, color }: { status: string; color: string }) {
    ...
    return (
        <span className={`px-2 py-1 rounded-md text-xs font-bold border ${colorMap[color] || colorMap.gray}`}>
            {status.replace(/_/g, " ")}
        </span>
    );
}
...
export default function KeuanganPage() {
    ...
    const [pendaftaranSummary, setPendaftaranSummary] = useState<PendaftaranSummary | null>(null);
    ...
    // Filter
    const filteredPendaftaran = pendaftaranData.filter(item =>
        item.nama.toLowerCase().includes(search.toLowerCase()) ||
        item.nomor_pendaftaran.includes(search)
    );

    const filteredDaftarUlang = daftarUlangData.filter(item =>
        item.nama.toLowerCase().includes(search.toLowerCase()) ||
        item.nomor_pendaftaran.includes(search)
    );
                    ...
                    {pendaftaranSummary && (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                                <p className="text-xs text-slate-500 font-medium mb-1">Total Pendaftar</p>
                                <p className="text-2xl font-black text-slate-800">{pendaftaranSummary.total}</p>
                            </div>
                            <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100 shadow-sm">
                                <div className="flex items-center gap-1.5 mb-1">
                                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                                    <p className="text-xs text-emerald-700 font-medium">Terverifikasi</p>
                                </div>
                                <p className="text-2xl font-black text-emerald-700">{pendaftaranSummary.terverifikasi}</p>
                            </div>
                            <div className="bg-amber-50 rounded-xl p-4 border border-amber-100 shadow-sm">
                                ...
                                <p className="text-2xl font-black text-amber-700">{pendaftaranSummary.menunggu}</p>
                            </div>
                            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 shadow-sm">
                                <div className="flex items-center gap-1.5 mb-1">
                                    <AlertCircle className="w-3.5 h-3.5 text-slate-500" />
                                    <p className="text-xs text-slate-500 font-medium">Belum Upload</p>
                                </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {filteredPendaftaran.length > 0 ? filteredPendaftaran.map(row => (
                                            <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                                                <td className="px-6 py-3 text-center text-slate-400">{row.no}</td>
                                                <td className="px-6 py-3 font-medium text-slate-900">
                                                    {row.nama}
                                                    <div className="text-xs text-slate-400 font-normal">{row.nomor_pendaftaran}</div>
                                                </td>
                                                <td className="px-6 py-3">
                                                    <StatusBadge status={row.status_pembayaran} color={row.status_color} />
                                                </td>
                                                <td className="px-6 py-3 font-mono text-slate-700">
                                                    {row.jumlah_pembayaran > 0 ? formatCurrency(row.jumlah_pembayaran) : "-"}
                                                </td>
                                                <td className="px-6 py-3 text-slate-500 capitalize">{row.metode}</td>
                                                <td className="px-6 py-3 text-xs text-slate-400">
                                                    {new Date(row.last_updated).toLocaleDateString("id-ID")}
                                                </td>
                                            </tr>
                                        )) : (
                                            <tr>
            ...
            {activeTab === "daftar-ulang" && (
                <div className="space-y-5">
                    <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 text-sm text-amber-800 font-medium flex items-center gap-2">
                        ...
                        Menampilkan rekap santri yang <strong>lulus seleksi</strong> dan status pembayaran daftar ulang mereka.
                    </div>
                        ...
                        {loadingDaftarUlang ? (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {filteredDaftarUlang.length > 0 ? filteredDaftarUlang.map(row => (
                                            <tr key={row.no} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-3">
                                                    <span className={`px-2 py-1 rounded-md text-xs font-bold border ${row.tipe_cicilan === "LUNAS"
