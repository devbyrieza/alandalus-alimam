...
const StatWidget = ({ label, value, icon: Icon, color, trend, breakdown }: any) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      className="bg-white p-8 rounded-[2.5rem] border border-cream-200 shadow-premium-sm hover:shadow-premium-lg transition-all group relative overflow-hidden">
      <div className="absolute top-0 right-0 w-32 h-32 bg-cream-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:bg-cream-100 transition-colors duration-500" />
      <div className="relative z-10">
        ...
        <div className="mb-8">
          <p className="text-[11px] font-black text-maroon-400 uppercase tracking-[0.25em] mb-2">{label}</p>
          <div className="flex items-baseline gap-2">
            <h3 className="text-5xl font-black text-maroon-900 tracking-tighter italic">{value}</h3>
            <span className="text-xs font-bold text-maroon-300">Orang</span>
          </div>
        </div>
        {breakdown && (
          <div className="grid grid-cols-2 gap-4 pt-6 border-t border-cream-200">
            <div className="space-y-4">
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-maroon-400 uppercase tracking-widest mb-1">MTs Putra</span>
                <span className="text-base font-black text-maroon-700 leading-none">{breakdown.mts_l || 0}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-maroon-400 uppercase tracking-widest mb-1">MTs Putri</span>
                <span className="text-base font-black text-pink-500 leading-none">{breakdown.mts_p || 0}</span>
              </div>
            </div>
            <div className="space-y-4 border-l border-cream-200 pl-4">
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-maroon-400 uppercase tracking-widest mb-1">IL Putra</span>
                <span className="text-base font-black text-maroon-700 leading-none">{breakdown.il_l || 0}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-maroon-400 uppercase tracking-widest mb-1">IL Putri</span>
...
export default function AdminDashboardPage() {
  ...
  if (loading && stats.total_pendaftar === 0) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-sm font-bold text-maroon-400 tracking-widest animate-pulse uppercase">Sinkronisasi Data...</p>
    </div>
  );
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-maroon-900 tracking-tight italic">Dasbor Al-Imam</h1>
          <p className="text-sm text-maroon-500 font-medium mt-1">Pantau perkembangan pendaftaran santri secara langsung.</p>
        </div>
        <button onClick={fetchStats} className="p-3 bg-white border border-cream-200 rounded-2xl text-maroon-400 hover:text-maroon-600 transition-all shadow-premium-sm">
          <RefreshCw className={`w-5 h-5 ${loading ? "animate-spin" : ""}`} />
        </button>
      </div>

      <div className="bg-maroon-900 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl border border-white/5">
          ...
          <div className="max-w-xl">
            ...
            <div className="flex items-center gap-12 mt-12">
              <div>
                ...
                <p className="text-[11px] font-black text-cream-300 uppercase tracking-widest mt-2">Pendaftar</p>
              </div>
              <div className="w-px h-16 bg-white/20" />
              <div>
                <span className="text-5xl font-black text-gold-400 italic">{stats.sudah_bayar}</span>
                <p className="text-[11px] font-black text-cream-300 uppercase tracking-widest mt-2">Tervalidasi</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-6 w-full lg:w-auto">
            <div className="bg-white/5 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10 flex flex-col items-center">
              <p className="text-[10px] font-black text-cream-300 uppercase tracking-widest mb-4">Lulus Seleksi</p>
              <p className="text-4xl font-black text-emerald-400 italic">{stats.diterima}</p>
            </div>
            <div className="bg-white/5 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10 flex flex-col items-center">
              <p className="text-[10px] font-black text-cream-300 uppercase tracking-widest mb-4">Daftar Ulang</p>
              <p className="text-4xl font-black text-gold-400 italic">{stats.daftar_ulang}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {!isAdminSuper && (
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-8">
                <div>
                  <p className="text-[10px] font-black text-cream-300 uppercase tracking-widest mb-2">Total Lunas</p>
                  <p className="text-4xl font-black text-white italic">{stats.sudah_bayar}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-cream-300 uppercase tracking-widest mb-2">Data Komplit</p>
                  <p className="text-4xl font-black text-cream-400 italic">{stats.sudah_isi_data}</p>
                </div>
              </div>
              <div className="space-y-8 pl-10 border-l border-white/20">
                <div>
                  <p className="text-[10px] font-black text-cream-300 uppercase tracking-widest mb-2">Antrean Aktif</p>
                  <p className="text-4xl font-black text-amber-300 italic">{stats.waiting_payment + stats.waiting_docs}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-cream-300 uppercase tracking-widest mb-2">Tingkat Kelulusan</p>
        ...
        <div className="bg-white rounded-[2.5rem] border border-cream-200 p-10 shadow-premium-sm relative overflow-hidden group">
          <div className="relative z-10">
            <h3 className="text-2xl font-black text-maroon-900 mb-8 tracking-tight flex items-center gap-3 italic">
              <div className="w-2 h-8 bg-maroon-600 rounded-full" />
              Aksi Cepat
            </h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-6 bg-cream-50 rounded-3xl border border-cream-200 hover:border-maroon-200 transition-all cursor-default group/item">
                <div className="flex items-center gap-5">
                  <div className="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center text-maroon-600 transition-transform group-hover/item:scale-110">
                    <Users className="w-6 h-6" />
                  </div>
                  <div>
                    <p className="text-sm font-black text-maroon-900 uppercase tracking-tighter leading-none mb-1">Cek Dokumen</p>
                    <p className="text-[11px] text-maroon-400 font-bold">Verifikasi berkas santri baru</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-2xl font-black text-maroon-900 italic">{stats.waiting_docs}</span>
                  <ChevronRight className="w-5 h-5 text-maroon-300" />
                </div>
              </div>
              <div className="flex items-center justify-between p-6 bg-cream-50 rounded-3xl border border-cream-200 hover:border-emerald-200 transition-all cursor-default group/item">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <p className="text-sm font-black text-maroon-900 uppercase tracking-tighter leading-none mb-1">Cek Pembayaran</p>
                    <p className="text-[11px] text-maroon-400 font-bold">Konfirmasi bukti transfer</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-2xl font-black text-maroon-900 italic">{stats.waiting_payment}</span>
                  <ChevronRight className="w-5 h-5 text-maroon-300" />
                </div>
              </div>
            </div>
