...
const ContactInfoCard = ({
  ...
  <motion.div
    ...
    className="app-card bg-white p-6 sm:p-8 rounded-2xl sm:rounded-[2.5rem] border border-maroon-100 shadow-sm hover:shadow-md transition-all group flex flex-col items-center text-center"
  >
    <div className="w-16 h-16 rounded-2xl bg-maroon-50 flex items-center justify-center text-maroon-600 mb-6 group-hover:scale-110 transition-transform shadow-sm border border-maroon-100">
      <Icon className="w-8 h-8" />
    </div>
    <h3 className="text-xl font-display font-black text-maroon-950 mb-3">
      {title}
    </h3>
    {href ? (
      <a
        href={href}
        className="text-lg font-bold text-maroon-600 hover:text-maroon-700 transition-colors break-all"
      >
        {content}
      </a>
    ) : (
...
const SocialCard = ({ social, delay = 0 }: { social: any; delay?: number }) => (
  <motion.a
    ...
    className={`app-card group flex items-center gap-4 sm:gap-5 p-4 sm:p-5 rounded-2xl sm:rounded-[2rem] bg-white border border-maroon-50 shadow-sm hover:shadow-md transition-all`}
  >
    <div
      className={`w-14 h-14 rounded-2xl ${social.bgLight} flex items-center justify-center ${social.textColor} group-hover:scale-110 transition-transform shadow-sm border border-maroon-50`}
    >
        ...
        <Container>
          <div className="grid lg:grid-cols-5 gap-10 lg:gap-24 items-start">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              className="lg:col-span-3 bg-white p-6 md:p-16 rounded-[2.5rem] md:rounded-[4rem] shadow-lg border border-maroon-50 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-96 h-96 bg-maroon-50/50 rounded-full blur-[100px] translate-x-1/2 -translate-y-1/2" />
                ...
                <AnimatePresence>
                  {showSuccess && (
                    <motion.div
                      ...
                      className="bg-maroon-50 border border-maroon-100 p-6 rounded-[2rem] overflow-hidden"
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-maroon-500 rounded-full flex items-center justify-center text-white shrink-0">
                          <CheckCircle className="w-6 h-6" />
                        </div>
                        <p className="text-maroon-900 font-black text-lg">
                ...
                >
                  <div className="grid md:grid-cols-2 gap-6 md:gap-8">
                    <div className="space-y-2 md:space-y-3">
                      ...
                      <input
                        ...
                        required
                        className="w-full px-5 py-3 md:px-8 md:py-5 rounded-xl md:rounded-2xl bg-maroon-50 border border-transparent focus:bg-white focus:border-maroon-200 focus:ring-4 focus:ring-maroon-50 transition-all font-bold text-ink-900 placeholder:text-ink-300 text-sm md:text-base"
                  ...
                  <div className="space-y-2 md:space-y-3">
                    ...
                    <textarea
                      ...
                      required
                      rows={5}
                      className="w-full px-5 py-3 md:px-8 md:py-5 rounded-xl md:rounded-2xl bg-maroon-50 border border-transparent focus:bg-white focus:border-maroon-200 focus:ring-4 focus:ring-maroon-50 transition-all font-bold text-ink-900 placeholder:text-ink-300 text-sm md:text-base resize-none"
                  ...
                  <motion.button
                    ...
                    className="w-full py-4 md:py-6 rounded-pill bg-maroon-900 text-white font-black text-lg md:text-xl hover:bg-gold-600 shadow-md transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                  >
            ...
            <div className="lg:col-span-2 space-y-8 md:space-y-12">
              ...
              >
                <div className="rounded-[2rem] md:rounded-[2.5rem] overflow-hidden h-[300px] md:h-[400px] relative border border-surface-50">
                  ...
                  />
                  <div className="absolute bottom-3 left-3 md:bottom-6 md:left-6 bg-white/90 backdrop-blur-sm px-3 py-1.5 md:px-6 md:py-3 rounded-lg md:rounded-2xl shadow-md border border-maroon-100 flex items-center gap-1.5 md:gap-3 z-10">
      ...
      <section className="py-16 md:py-24 lg:py-32 bg-white">
        <Container>
          <motion.div
            ...
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            className="bg-maroon-950 rounded-[2rem] md:rounded-[3rem] lg:rounded-[4rem] p-6 sm:p-8 md:p-16 lg:p-24 text-center text-white relative overflow-hidden shadow-premium-2xl"
          >
            ...
            <div className="relative z-10">
              <h2 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-display font-black mb-4 sm:mb-6 md:mb-8 text-white leading-tight">
                Mari Bergabung <br />{" "}
                <span className="text-cream-100">Menjadi Keluarga</span>
              </h2>
              <p className="text-sm sm:text-base md:text-lg lg:text-xl text-maroon-50 max-w-2xl mx-auto mb-6 sm:mb-8 md:mb-10 lg:mb-12 font-medium px-2">
